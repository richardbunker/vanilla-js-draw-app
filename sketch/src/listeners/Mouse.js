...
export const mouseDown = ({ canvas, state }) => {
  canvas.addEventListener("mousedown", (event) => onMouseDown(event, canvas, state), false);
};
...
export const mouseUpOut = ({ canvas, context, state, toolBar }) => {
  canvas.addEventListener("mouseup", () => onMouseUp(canvas, context, state, toolBar), false);
  canvas.addEventListener("mouseout", () => onMouseUp(canvas, context, state, toolBar), false);
};
const onMouseDown = (event, canvas, state) => {
  if (event.button == 2) {
...
const onMouseMove = (event, canvas, context, state, toolBar) => {
  ...
  if (state.panning) {
    ...
    if (!state.erasing) {
      state.currentStroke.push([toTrueX(state.cursorX, state), toTrueY(state.cursorY, state)]);
      drawLine(
        context,
        state.cursorXprev,
        state.cursorYprev,
      ...
      drawEraser(context, state.cursorXprev, state.cursorYprev, state.cursorX, state.cursorY);
      findAndRemoveStrokeFromHistory(
        canvas,
        context,
        state,
