...
import { hidePenMenu } from "./ToolBar.js";
...
export const drawStroke = (context, state, { vectors, colour, lineWidth }) => {
  ...
  context.beginPath();
  context.moveTo(toScreenX(vectors[0][0], state), toScreenY(vectors[0][1], state));
...
export const findAndRemoveStrokeFromHistory = (canvas, context, state, toolBar, x0, y0) => {
  ...
  if (foundXY) {
    state.actionHistory.push(removeFromStrokeHistory(state.strokeHistory, strokeIndex));
...
const xyExistsInStroke = (vectors, x, y) => {
  ...
  let foundStrokeX = false;
  let foundStrokeY = false;
  if (vectorYs[0] <= eraserMatrix[1][20] && eraserMatrix[1][0] <= vectorYs[vectorYs.length - 1]) {
    foundStrokeY = true;
  }
  if (vectorXs[0] <= eraserMatrix[0][20] && eraserMatrix[0][0] <= vectorXs[vectorXs.length - 1]) {
    foundStrokeX = true;
  }
  if (foundStrokeX && foundStrokeY) {
    return true;
...
const xyExistsInQuarteredStroke = (vectors, x, y) => {
  ...
  quarteredVectors.map((quarteredVector) => {
    ...
    const vectorXs = quarteredVector.map((vector) => vector[0]).sort((a, b) => a - b);
    const vectorYs = quarteredVector.map((vector) => vector[1]).sort((a, b) => a - b);
    if (vectorYs[0] <= eraserMatrix[1][20] && eraserMatrix[1][0] <= vectorYs[vectorYs.length - 1]) {
...
const halfableArray = (array) => {
  ...
  for (let index = 0; index < length; index++) {
    index < halfWayPoint ? firstHalf.push(array[index]) : secondHalf.push(array[index]);
  }
...
export const updateStrokeHistory = (strokeHistory, currentStroke, penColour, lineWidth, scale) => {
  strokeHistory.push({
